...
const menuItems = ref([
    ...
    },
    {
        ...
        icon: 'pi pi-trash',
        command: () => {
            Manager.Hosts.value = Manager.Hosts.value.filter(entry => entry !== selectedEntry.value);
        }
    }
]);
...
<template>
    ...
    <div class="surface-card m-2 p-2 shadow-2 border-round">
        <ul class="list-none p-0 m-0">
            <li class="surface-border" v-for="entry in Manager.Hosts" @contextmenu="managementMenu($event, entry)">
                <div class="border-round surface-0 hover:surface-100 p-1 my-1 mx-1 grid"
